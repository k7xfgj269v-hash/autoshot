...
# ── DeepSeek Prompt 模板 ──────────────────────────────────
SCRIPT_PROMPT_MODE_A = """你是一个短视频文案专家，擅长创作吸引眼球、直击痛点的竖屏短视频脚本。

请根据主题内容生成：
1. 视频标题（20字以内，有冲击力）
2. 口播文案（60-90秒，开头5秒必须抓住注意力）
3. 字幕文本列表（和口播一致，分句断行）
4. 话题标签列表（10个，贴合内容方向）
5. 封面文案（10字以内）
...
SCRIPT_PROMPT_MODE_B = """你是一个短视频文案专家。根据主题内容、原视频转写文本和场景描述，生成优化后的视频脚本。
